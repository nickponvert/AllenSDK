...
def get_extended_stimulus_presentations(session):
    ...
    if 'omitted' in intermediate_df['image_name'].unique():
...
if __name__ == "__main__":
    ...
    if case == 0:
        ...
        api = bpc.PickleFileApi(behavior_session_id)
        session = BehaviorOphysSession(api)

        output_path = "/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/SWDB_2019/pickle_file_extended_stim"

        edf = get_extended_stimulus_presentations(session)

        output_fn = os.path.join(
            output_path, "extended_stimulus_presentations_df_behavior_session_{}.h5".format(behavior_session_id)
        )
        print("Writing extended_stimulus_presentations_df to {}".format(output_fn))
        edf.to_hdf(output_fn, key="df")

    elif case == 1:
    
        #  bsid = 768450504
        #  bsid = 800919728
        #  bsid = 762418364 vsig/time?
        bsid = 791877093
        api = bpc.PickleFileApi(bsid)
        session = BehaviorOphysSession(api)
        edf = get_extended_stimulus_presentations(session)
        ...
        for donor_id in donor_ids:
            ...
            for bsid in behavior_session_ids:
                total.append(bsid)
                epath = cache.get_pickle_file_extended_stimulus_presentations_df(bsid)
        ...
        with open('/home/nick.ponvert/failure_log.txt', 'w') as logf:
            for failure_id in failure:
                
                api = bpc.PickleFileApi(failure_id)
                session = BehaviorOphysSession(api)
                try:
                    edf = get_extended_stimulus_presentations(session)
                except Exception as e:
                    print("Failed on {}".format(failure_id))
                    logf.write("Failed on bsid {0}: {1}\n".format(str(failure_id), str(e)))
